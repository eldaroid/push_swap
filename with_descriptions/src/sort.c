...
/*   Updated: 2020/10/04 18:40:46 by fgracefo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	st_b_to_a(t_info *s)
{
	...
	while (s->b)
	{
		...
		min_cmds = st_set_tmp_cmds(s, tmp_b); //число минимальных комманд чтобы перекинуть из стэка б в стэк б
...
void	st_a_min_to_top(t_info *s)
{
	...
	tmp_a = s->a;
	min_node = tmp_a;
	while (tmp_a) //ищет самое минимальное число
	{
